...
class DualTransformer(nn.Module):
    def __init__(
        ...
        self.title_attn = nn.Linear(embed_dim, 1)
        self.diff_attn = nn.Linear(embed_dim, 1)
    ...
    def _pool(self, encoded, key_padding_mask, attn_layer):
        ...
        scores = attn_layer(encoded).squeeze(-1)
        scores = scores.masked_fill(key_padding_mask, -1e9)
        weights = torch.softmax(scores, dim=1)
        weights = weights.masked_fill(key_padding_mask, 0.0)
        attn_pool = torch.sum(encoded * weights.unsqueeze(-1), dim=1)

        return mean_pool + attn_pool
    ...
    def forward(self, title, diff):
        ...
        title_vec = self._pool(title_out, title_mask, self.title_attn)
        diff_vec = self._pool(diff_out, diff_mask, self.diff_attn)
...
@register_detector("TRANSFORMER_V2")
class TransformerV2Detector(DefectDetector):
    def load(self):
        if self._loaded:
            return
